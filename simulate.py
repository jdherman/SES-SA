...
class simulate_SES:
  def __init__(self, r, R_max, a, b1, b2, q, c, d, k, p, h, g, m,
    ...
    self.r = r # resource regeneration rate
    self.R_0 = R_0 #resource
    self.R_max = R_max #total resource amount
    self.a = a # revenue threshold
    self.b1 = b1 # share parameter associated with resource
    self.b2 = b2 # share parameter associated with labor
    self.q = q # substituttion parameter 
    self.c = c # extraction cost parameter
    self.d = d # fixed cost
    self.k = k # benefit of resource access in exponential parameter
    self.p = p # profit
    self.h = h # parameter modulating rate at which wage increases
    self.g = g # parameter modulating rate at which wage decreases
    self.m = m # responsiveness of domestic users to payoff difference
    self.W_0 = W_0
    self.W_min = W_min
    self.U_0 = U_0
    self.dt = dt
  ...
  def profit_without_fee(self, inputs, R, W):
    # Inputs should be an array with E resource extration) and L (labor) as elements.
    E = inputs[0]
    L = inputs[1]
    if E <= 0 or L <= 0:
      try:
  ...
  def replicator(self, U, L, S, W): # Domestic User Well-being Function
    if U > 0:
  ...
  def wage_function(self, W, L, U, marg_benefit): # To represent supply and demand in wages and labor
